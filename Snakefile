...
TESTSET_SOFTMAX_MIA = [
    *expand(
        "softmax-mia/{N}x{acc_exp}xf{precision}/{variant}",
        N=range(16, 129, 16),
        acc_exp=[1, 2, 3, 4, 5, 6],
        precision=[16, 32, 64],
        variant=["linalg_xdsl"],
...
def cc_link_inputs(wildcards):
    ...
    # softmax-mia provides its softmax orchestration as a separate
    # translation unit that calls the xDSL-lowered exp_kernel.
...
def get_exp_attrs_from_variant(wildcards):
    ...
    Chebyshev variants (linalg_xdsl_cN): {degree = N : i64, lower = -10.0 : f64, upper = 0.0 : f64}
    """
    import re
    ...
    if m:
        return f"degree = {m.group(1)} : i64, lower = -10.0 : f64, upper = 0.0 : f64"
