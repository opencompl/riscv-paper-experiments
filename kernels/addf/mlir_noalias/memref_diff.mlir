func.func @memref_diff(%A: memref<13x26xf32>{llvm.noalias},
                       %B: memref<13x26xf32>{llvm.noalias},
                       %eps: f32) -> i32 {
  %i0 = arith.constant 0 : index
  %i1 = arith.constant 1 : index
  %m = memref.dim %A, %i0 : memref<13x26xf32>
  %n = memref.dim %A, %i1 : memref<13x26xf32>
  ...
  iter_args(%m_partial = %0) -> (i32) {
    ...
    iter_args(%n_partial = %0) -> (i32) {
      %va = memref.load %A[%m_i, %n_i] : memref<13x26xf32>
      %vb = memref.load %B[%m_i, %n_i] : memref<13x26xf32>
