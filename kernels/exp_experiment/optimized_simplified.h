...
__thread const double EXP_N = 1 << EXP2F_TABLE_BITS;
__thread const double InvLn2N = 0x1.71547652b82fep+0 * EXP_N;
__thread const double SHIFT = 0x1.8p+52;
__thread const double C[4] = {0x1.c6af84b912394p-5 / EXP_N / EXP_N / EXP_N,
                              0x1.ebfce50fac4f3p-3 / EXP_N / EXP_N,
...
static inline void exp_optimized(double *a, double *b) {
    ...
    double EXP_N = 1 << EXP2F_TABLE_BITS;
    double InvLn2N = 0x1.71547652b82fep+0 * EXP_N;
    double SHIFT = 0x1.8p+52;
    double C[4] = {0x1.c6af84b912394p-5 / EXP_N / EXP_N / EXP_N,
                   0x1.ebfce50fac4f3p-3 / EXP_N / EXP_N, 0x1.62e42ff0c52d6p-1 / EXP_N, 1.0};
    ...
    if (snrt_is_dm_core()) {
        ...
        // DMA transfer
        snrt_dma_load_1d_tile(dma_a_ptr, a, 0, BATCH_SIZE,
                                sizeof(double));
    ...
    // dma_a_idx = 1;
    // contents of a loaded into a_buffers[0]
    ...
    // fp0_w_idx = 1, fp0_a_idx = 1
    // results of FP0 phase stored in kd_buffers[0] and w_buffers[0]
    ...
    // fp1_w_idx = 1, fp1_t_idx = 1
    // read from w_buffers[0] and t_buffers[0], write to b_buffers[0]
    ...
    if (snrt_is_dm_core()) {
        ...
        // DMA transfer
        snrt_dma_store_1d_tile(b, dma_b_ptr, 0, BATCH_SIZE,
                                sizeof(double));
