...
static inline void vexpf_optimized_v2(double *a, double *b) {
    ...
    for (int iteration = 0; iteration < n_iterations; iteration++) {
        // NOTE: The upstream code has `snrt_mcycle()` here for per-iteration
        // profiling. We omit it because the riscv-paper-experiments profiling
        // infrastructure (trace_to_profile.py) expects exactly 3 mcycle
        // sections in the trace (before kernel, kernel, after kernel).
        // Per-iteration mcycle calls would create extra sections and break
