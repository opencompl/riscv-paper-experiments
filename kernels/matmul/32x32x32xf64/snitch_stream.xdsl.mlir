...
// g[ M x N ]
  riscv_func.func @matmul(
    ...
    %frep_count_minus_one = riscv.li 29 : () -> !riscv.reg<>
    %target_count = riscv.li 1024 : () -> !riscv.reg<>

    "snitch_stream.streaming_region"(%X_moved, %Y_moved, %G_moved) <{
      "stride_patterns" = [
        #snitch_stream.stride_pattern<ub = [32, 8, 32, 4], strides = [256, 0, 8, 0]>,
        #snitch_stream.stride_pattern<ub = [32, 8, 32, 4], strides = [0, 32, 256, 8]>,
        #snitch_stream.stride_pattern<ub = [1024], strides = [8]>
      ],
