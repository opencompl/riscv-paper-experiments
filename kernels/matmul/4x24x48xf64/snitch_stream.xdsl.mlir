...
// g[ M x N ]
  riscv_func.func @matmul(
    ...
    %frep_count_minus_one = riscv.li 21 : () -> !riscv.reg<>
    %target_count = riscv.li 192 : () -> !riscv.reg<>

    "snitch_stream.streaming_region"(%X_moved, %Y_moved, %G_moved) <{
      "stride_patterns" = [
        #snitch_stream.stride_pattern<ub = [4, 12, 24, 4], strides = [192, 0, 8, 0]>,
        #snitch_stream.stride_pattern<ub = [4, 12, 24, 4], strides = [0, 32, 384, 8]>,
        #snitch_stream.stride_pattern<ub = [192], strides = [8]>
      ],
