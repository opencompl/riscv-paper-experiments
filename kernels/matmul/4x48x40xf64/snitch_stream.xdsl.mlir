...
// g[ M x N ]
  riscv_func.func @matmul(
    ...
    %frep_count_minus_one = riscv.li 45 : () -> !riscv.reg<>
    %target_count = riscv.li 160 : () -> !riscv.reg<>

    "snitch_stream.streaming_region"(%X_moved, %Y_moved, %G_moved) <{
      "stride_patterns" = [
        #snitch_stream.stride_pattern<ub = [4, 10, 48, 4], strides = [384, 0, 8, 0]>,
        #snitch_stream.stride_pattern<ub = [4, 10, 48, 4], strides = [0, 32, 320, 8]>,
        #snitch_stream.stride_pattern<ub = [160], strides = [8]>
      ],
