...
// g[ M x N ]
  riscv_func.func @matmul(
    ...
    %frep_count_minus_one = riscv.li 57 : () -> !riscv.reg<>
    %target_count = riscv.li 208 : () -> !riscv.reg<>

    "snitch_stream.streaming_region"(%X_moved, %Y_moved, %G_moved) <{
      "stride_patterns" = [
        #snitch_stream.stride_pattern<ub = [4, 13, 60, 4], strides = [480, 0, 8, 0]>,
        #snitch_stream.stride_pattern<ub = [4, 13, 60, 4], strides = [0, 32, 416, 8]>,
        #snitch_stream.stride_pattern<ub = [208], strides = [8]>
      ],
