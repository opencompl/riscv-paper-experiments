...
// g[ M x N ]
  riscv_func.func @matmul(
    ...
    %frep_count_minus_one = riscv.li 17 : () -> !riscv.reg<>
    %target_count = riscv.li 384 : () -> !riscv.reg<>

    "snitch_stream.streaming_region"(%X_moved, %Y_moved, %G_moved) <{
      "stride_patterns" = [
        #snitch_stream.stride_pattern<ub = [8, 12, 20, 4], strides = [160, 0, 8, 0]>,
        #snitch_stream.stride_pattern<ub = [8, 12, 20, 4], strides = [0, 32, 384, 8]>,
        #snitch_stream.stride_pattern<ub = [384], strides = [8]>
      ],
