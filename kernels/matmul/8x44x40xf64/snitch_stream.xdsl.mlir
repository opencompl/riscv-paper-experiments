...
// g[ M x N ]
  riscv_func.func @matmul(
    ...
    %frep_count_minus_one = riscv.li 41 : () -> !riscv.reg<>
    %target_count = riscv.li 320 : () -> !riscv.reg<>

    "snitch_stream.streaming_region"(%X_moved, %Y_moved, %G_moved) <{
      "stride_patterns" = [
        #snitch_stream.stride_pattern<ub = [8, 10, 44, 4], strides = [352, 0, 8, 0]>,
        #snitch_stream.stride_pattern<ub = [8, 10, 44, 4], strides = [0, 32, 320, 8]>,
        #snitch_stream.stride_pattern<ub = [320], strides = [8]>
      ],
