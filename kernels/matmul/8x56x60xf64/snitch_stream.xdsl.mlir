...
// g[ M x N ]
  riscv_func.func @matmul(
    ...
    %frep_count_minus_one = riscv.li 53 : () -> !riscv.reg<>
    %target_count = riscv.li 480 : () -> !riscv.reg<>

    "snitch_stream.streaming_region"(%X_moved, %Y_moved, %G_moved) <{
      "stride_patterns" = [
        #snitch_stream.stride_pattern<ub = [8, 15, 56, 4], strides = [448, 0, 8, 0]>,
        #snitch_stream.stride_pattern<ub = [8, 15, 56, 4], strides = [0, 32, 480, 8]>,
        #snitch_stream.stride_pattern<ub = [480], strides = [8]>
      ],
