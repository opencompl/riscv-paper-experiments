...
// g[ M x N ]
  riscv_func.func @matmul(
    ...
    %frep_count_minus_one = riscv.li 61 : () -> !riscv.reg<>
    %target_count = riscv.li 448 : () -> !riscv.reg<>

    "snitch_stream.streaming_region"(%X_moved, %Y_moved, %G_moved) <{
      "stride_patterns" = [
        #snitch_stream.stride_pattern<ub = [8, 14, 64, 4], strides = [512, 0, 8, 0]>,
        #snitch_stream.stride_pattern<ub = [8, 14, 64, 4], strides = [0, 32, 448, 8]>,
        #snitch_stream.stride_pattern<ub = [448], strides = [8]>
      ],
