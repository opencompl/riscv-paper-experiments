#!/usr/bin/env python3
"""
Plot exp kernel results for Chebyshev variants across all precisions.

Usage:
    python plot_exp_micro_chebyshev.py --input ../results/kernels.exp_micro.csv --output output/exp_chebyshev_plots.pdf
"""
...
def load_and_prepare(csv_path: str) -> pd.DataFrame:
    ...
    # Keep baseline and all chebyshev variants (c4, c8, c12, c16)
    chebyshev_impls = df["impl"].str.match(r"linalg_xdsl_c\d+") | (df["impl"] == "baseline")
    df = df[chebyshev_impls].copy()
...
def main():
    parser = argparse.ArgumentParser(description="Plot exp kernel results (Chebyshev variants, all precisions)")
    parser.add_argument(
        "--input", "-i",
    ...
    parser.add_argument(
        "--output", "-o",
        default="output/exp_chebyshev_plots.pdf",
