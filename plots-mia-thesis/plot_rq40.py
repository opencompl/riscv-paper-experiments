#!/usr/bin/env python3
"""
RQ40 plot: realised mantissa bits lost in the softmax *output* as a
...
(matches plot_rq42's convention).
...
Usage:
    python plot_rq40.py [--n 64] [--samples 100] [--seed 0] \\
        [-o plots-mia-thesis/output/rq40_plots.pdf]
"""
...
def plot_rq40(n: int, samples: int, seed: int) -> plt.Figure:
...
def main() -> None:
    ...
    parser.add_argument(
        "--output", "-o",
        default="plots-mia-thesis/output/rq40_plots.pdf",
        help="Output plot file",
    )
    args = parser.parse_args()

    Path(args.output).parent.mkdir(parents=True, exist_ok=True)
    fig = plot_rq40(args.n, args.samples, args.seed)
