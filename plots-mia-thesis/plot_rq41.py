#!/usr/bin/env python3
"""
RQ41 plot: per-element comparison of correctly-rounded exp(x_i) against the
degree-d Chebyshev-Lobatto approximation, for d = 2..12 and three input
vectors per precision (f16, f32, f64).
...
in the Snakefile, so the curves here correspond exactly to what the xDSL
pipeline emits.
...
curve per degree d in [CHEBYSHEV_DEGREES][2..12].

Usage:
    python plot_rq41.py [--n 16] [--seeds 0 1 2] \\
...
def chebyshev_approx(x_prime: np.ndarray, degree: int, precision: str) -> np.ndarray:
    """Degree-d Chebyshev-Lobatto interpolant of exp on [-2, 0], evaluated in
    float64 then rounded to `precision` to match the dtype of the kernel."""
    dtype = PRECISION_NUMPY_DTYPE[precision]
    approx = eval_chebyshev(chebyshev_lobatto_coeffs(degree), x_prime.astype(np.float64))
    return approx.astype(dtype)


def plot_rq41(n: int, seeds: list[int]) -> plt.Figure:
...
def main() -> None:
    ...
    fig = plot_rq41(args.n, args.seeds)
