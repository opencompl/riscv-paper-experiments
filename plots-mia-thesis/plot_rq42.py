#!/usr/bin/env python3
"""
RQ42 plot: per-precision performance of the softmax_polynomial kernel as a
...
one column per precision (f16/f32/f64), one row per metric:
  - row 1: avg cycles / byte    (lower = faster)
  - row 2: avg FPU utilization  (column `fpss_fpu_occupancy` in the CSV)
...
max_bits_lost, and averages each metric across the input sizes that were
...
Usage:
    python plot_rq42.py [-i results/kernels.softmax_polynomial.csv] \\
        [-o plots-mia-thesis/output/rq42_plots.pdf]
"""
...
def plot_rq42(poly_df: pd.DataFrame, baseline_df: pd.DataFrame) -> plt.Figure:
    fig, axes = plt.subplots(
        len(METRICS), len(PRECISIONS),
        figsize=(len(PRECISIONS) * 4.5, len(METRICS) * 4.0),
        sharey=False,
    )
    ...
    for row_idx, (metric, ylabel) in enumerate(METRICS):
        for col_idx, precision in enumerate(PRECISIONS):
            ...
            series = avg_per_bits_lost(poly_df, precision, metric)
...
def main() -> None:
    ...
    fig = plot_rq42(poly_df, baseline_df)
    despine(fig)
